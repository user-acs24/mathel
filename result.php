                            ...
                            <div class="col-md-8 col-md-offset-2">
                                <div class="panel">
                                    ...
                                    <div class="panel-body p-20">
                                        ...
                                        $query = "(SELECT tblstudentss.StudentName, tblstudentss.RollId, tblstudentss.RegDate, tblstudentss.StudentId, tblstudentss.Status, tblclassess.ClassName, tblclassess.Section 
FROM tblstudentss
JOIN tblclassess ON tblclassess.id = tblstudentss.ClassId 
WHERE tblstudentss.RollId = :rollid AND tblstudentss.ClassId = :classid) ";
                                        ...
                                        if ($stmt->rowCount() > 0) {
                                            foreach ($resultss as $row) {
                                                ...
                                                <p><b>Reg. No :</b> <?php echo htmlentities($row->RollId); ?></p>
                                                <p><b>Department:</b> <?php echo htmlentities($row->ClassName); ?>(<?php echo htmlentities($row->Section); ?>)</p>
                                                <?php
                                            }
                                        }
                                        ?>
...
FROM (
    SELECT sts.StudentName, sts.RollId, sts.ClassId, tr.marks, tr.SubjectId 
    FROM tblstudents AS sts 
    JOIN tb_data AS tr ON tr.StudentId = sts.StudentId 
...
SELECT t.StudentName, t.RollId, t.ClassId, t.marks, t.SubjectId, tblsubjectss.SubjectName 
FROM (
    SELECT sts.StudentName, sts.RollId, sts.ClassId, tr.marks, tr.SubjectId 
    FROM tblstudentss AS sts 
    JOIN tb_data AS tr ON tr.StudentId = sts.StudentId 
    WHERE sts.RollId = :rollid AND sts.ClassId = :classid
) AS t 
JOIN tblsubjectss ON tblsubjectss.id = t.SubjectId 
WHERE (t.RollId = :rollid AND t.ClassId = :classid)
";
...
?>
                                        ...
                                        <?php if ($stmt->rowCount() == 0) { ?>
                                            <div class="alert alert-danger left-icon-alert" role="alert">
                                                ...
                                                <?php echo htmlentities("Invalid Register Number"); ?>
                                            </div>
                                        <?php } ?>
                                    </div>
                                </div>
