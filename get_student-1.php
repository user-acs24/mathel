...
if(!empty($_POST["classid"])) 
{
 ...
 $stmt = $dbh->prepare("SELECT StudentName,StudentId FROM tblstudentss WHERE ClassId= :id order by StudentName");
...
if(!empty($_POST["classid1"])) 
{
 ...
 $status=0;	
 $stmt = $dbh->prepare("SELECT tblsubjectss.SubjectName,tblsubjectss.id FROM tblsubjectcombinations join  tblsubjectss on  tblsubjectss.id=tblsubjectcombinations.SubjectId WHERE tblsubjectcombinations.ClassId=:cid and tblsubjectcombinations.status!=:stts order by tblsubjectss.SubjectName");
